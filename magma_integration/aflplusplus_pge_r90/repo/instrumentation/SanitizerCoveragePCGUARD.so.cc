...
const char PGEExitFuncName[] = "exit"; // @PGE: exit function
...
class ModuleSanitizerCoverageAFL
 ...
 private:
  ...
  GlobalVariable *PGECntPtr = NULL; // @PGE: execution counter
  GlobalVariable *PGECntUpPtr = NULL; // @PGE: execution counterUp
  FunctionCallee PGEExitFunc; // @PGE: exit function
  ConstantInt *   PGEOne = NULL; // @PGE: (u32)1
  ConstantInt *   PGEZero = NULL;// @PGE: (u32)0
  uint32_t pge_instr = 0;
...
bool ModuleSanitizerCoverageAFL::instrumentModule(
  ...
  if ((isatty(2) && !getenv("AFL_QUIET")) || debug) {
    ...
    SAYF(cCYA "SanitizerCoveragePCGUARD-PGE" VERSION cRST "\n");
  ...
  PGECntPtr = // @PGE: load pointer
      new GlobalVariable(M, PointerType::get(Int32Ty, 0), false,
                         GlobalValue::ExternalLinkage, 0, "__afl_area_cnt_ptr");
  PGECntUpPtr = // @PGE: load pointer
      new GlobalVariable(M, PointerType::get(Int32Ty, 0), false,
                         GlobalValue::ExternalLinkage, 0, "__afl_area_cntUp_ptr");
  PGEExitFunc = // @PGE: create PGE exit function
      M.getOrInsertFunction(PGEExitFuncName, VoidTy, IRB.getInt8Ty());
  PGEOne = ConstantInt::get(IntegerType::getInt32Ty(Ctx), 1);
  PGEZero = ConstantInt::get(IntegerType::getInt32Ty(Ctx), 0);
  ...
  if (!be_quiet) {
    ...
    if (!instr)
      ...
      OKF("PGE: Instrumented %u locations with no collisions.",
          pge_instr);
...
void ModuleSanitizerCoverageAFL::InjectCoverageAtBlock(Function &  F,
  ...
  if (Options.TracePCGuard) {
    ...
    if (R(100) < 100) {
      /* Increase counter */
      LoadInst *CntPtr = IRB.CreateLoad(PointerType::get(Int32Ty, 0), PGECntPtr);
      ModuleSanitizerCoverageAFL::SetNoSanitizeMetadata(CntPtr);
      Value *CntPtrIdx = IRB.CreateGEP(Int32Ty, CntPtr, PGEZero);
      LoadInst *Cnt = IRB.CreateLoad(IRB.getInt32Ty(), CntPtrIdx);
      ModuleSanitizerCoverageAFL::SetNoSanitizeMetadata(Cnt);
      Value *CntIncr = IRB.CreateAdd(Cnt, PGEOne);
      StoreInst *CntStoreCtx = IRB.CreateStore(CntIncr, CntPtrIdx);
      ModuleSanitizerCoverageAFL::SetNoSanitizeMetadata(CntStoreCtx);

      /* Decide if exceeds prefix */
      LoadInst *CntUpPtr = IRB.CreateLoad(PointerType::get(Int32Ty, 0), PGECntUpPtr);
      ModuleSanitizerCoverageAFL::SetNoSanitizeMetadata(CntUpPtr);
      Value *CntUpPtrIdx = IRB.CreateGEP(Int32Ty, CntUpPtr, PGEZero);
      LoadInst *CntUp = IRB.CreateLoad(IRB.getInt32Ty(), CntUpPtrIdx);
      ModuleSanitizerCoverageAFL::SetNoSanitizeMetadata(CntUp);

      auto IsExceedsCntUp = IRB.CreateICmpUGT(Cnt, CntUp);
      auto ThenTerm = SplitBlockAndInsertIfThen(IsExceedsCntUp, &*IP, false);
      IRBuilder<> ThenIRB(ThenTerm);
      ThenIRB.CreateCall(PGEExitFunc, PGEZero);
