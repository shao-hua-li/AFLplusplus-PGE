...
SKIP_TO_PGE: // @PGE: goto here if in PGE
  ...
  if (unlikely((!afl->pge.search_phase) && afl->shm.cmplog_mode &&
   ...
   *********************/
  if (!afl->pge.search_phase) {
    u64 search_start = get_cur_time();
    u8 res = pge_prefix_search(afl);
    u64 search_end = get_cur_time();
    afl->pge.search_time = afl->pge.search_time + (search_end - search_start);
    pge_log(afl, PGE_LOG_SEARCH, "END:");
    if (res == PGE_SEARCH_SUCC) {
      afl->pge.active = 1; // enter PGE
    } else {
      afl->pge.active = 0;
    }
    out_buf = afl_realloc(AFL_BUF_PARAM(out), len);
...
#define MUTATE_ASCII_DICT 64
  ...
  // @PGE: avoid changing fuzzing state
  if (afl->pge.search_phase)
    goto retry_splicing;
