...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault) {
  ...
  if (likely(fault == afl->crash_mode)) {
    ...
    /* @PGE: save the current trace_bits checksum as the following code may change it. */
    if (afl->pge.active || afl->pge.search_phase) {
      memcpy(afl->pge.curr_trace_bits, afl->fsrv.trace_bits, afl->fsrv.map_size);
      simplify_trace(afl, afl->pge.curr_trace_bits);
      afl->pge.curr_trace_cksum = hash64(afl->pge.curr_trace_bits, afl->fsrv.map_size, HASH_CONST);
    }
