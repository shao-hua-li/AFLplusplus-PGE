...
void afl_shm_deinit(sharedmem_t *shm) {
  ...
  // @PGE: unset env
  unsetenv(SHM_ENV_VAR_CNT);
  unsetenv(SHM_ENV_VAR_CNTUP);
...
u8 *pge_shm_init(sharedmem_t *shm, size_t map_size,
                 u8 is_cntUp) {
  ...
  /* If somebody is asking us to fuzz instrumented binaries in
      ...
      with better auto-detection later on, perhaps? */
  if (!is_cntUp)
    setenv(SHM_ENV_VAR_CNT, shm_str, 1);
  else
    setenv(SHM_ENV_VAR_CNTUP, shm_str, 1);
