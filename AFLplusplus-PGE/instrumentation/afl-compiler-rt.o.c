...
/* @PGE: pointers and values for counter */
u32  __afl_area_cnt_initial; //@PGE: for pc-guard
u32* __afl_area_cnt_ptr = &__afl_area_cnt_initial; //@PGE: for pc-guard
u32  __afl_area_cntUp_initial=UINT32_MAX; //@PGE: for pc-guard
u32* __afl_area_cntUp_ptr = &__afl_area_cntUp_initial; //@PGE: for pc-guard
u32 __afl_area_cnt_ptr_value=0;
u32 __afl_area_cntUp_ptr_value=UINT32_MAX;
u8 __afl_already_atexit = 0;
jmp_buf fuzz_jmp_buf;

/* @PGE: update counter value before exit. */
void update_cnt(void) {
  if (__afl_area_cnt_ptr[0] == 0)
    __afl_area_cnt_ptr[0] = __afl_area_cnt_ptr_value;
  // _exit(0);
}
...
static void __afl_map_shm(void) {
  ...
  // @PGE: necessary info.
    char *id_cnt_str = getenv(SHM_ENV_VAR_CNT);
    char *id_cntUp_str = getenv(SHM_ENV_VAR_CNTUP);
  #ifndef PGE_PERSISTENT
    if(!__afl_already_atexit) {
      atexit(update_cnt);
      __afl_already_atexit = 1;
    }
  #endif

  // @PGE: setup shared mem for counter
  if (id_cnt_str) {

    u32 shm_cnt_id = atoi(id_cnt_str);

    __afl_area_cnt_ptr = shmat(shm_cnt_id, NULL, 0);

    if (__afl_area_cnt_ptr == (void *)-1) _exit(1);

    __afl_area_cnt_ptr[0] = 1;
  }
  if (__afl_debug) {
      fprintf(stderr,
              "DEBUG: PGE(0) id_cnt_str %s, __afl_area_cnt_ptr %p, cnt=%u\n",
              id_cnt_str == NULL ? "<null>" : id_cnt_str, __afl_area_cnt_ptr, __afl_area_cnt_ptr[0]);
  }

  // @PGE: setup shared mem for counterUp
  if (id_cntUp_str) {

    u32 shm_cntUp_id = atoi(id_cntUp_str);

    __afl_area_cntUp_ptr = shmat(shm_cntUp_id, NULL, 0);

    if (__afl_area_cntUp_ptr == (void *)-1) _exit(1);
    // __afl_area_cntUp_ptr[0] = 1;
  }
  if (__afl_debug) {
      fprintf(stderr,
              "DEBUG: PGE(1) id_cntUp_str %s, __afl_area_cntUp_ptr %p, cntUp=%u\n",
              id_cntUp_str == NULL ? "<null>" : id_cntUp_str, __afl_area_cntUp_ptr, __afl_area_cntUp_ptr[0]);
  }
...
static void __afl_unmap_shm(void) {
  ...
  // @PGE: unmap
  id_str = getenv(SHM_ENV_VAR_CNT);
  if (id_str) {
    shmdt((void *)__afl_area_cnt_ptr);
    __afl_area_cnt_ptr = &__afl_area_cnt_initial;
  }
  id_str = getenv(SHM_ENV_VAR_CNTUP);
  if (id_str) {
    shmdt((void *)__afl_area_cntUp_ptr);
    __afl_area_cntUp_ptr = &__afl_area_cntUp_initial;
  }
...
static void __afl_start_forkserver(void) {
  ...
  while (1) {
    ...
    __afl_area_cntUp_ptr_value = __afl_area_cntUp_ptr[0];
    __afl_area_cnt_ptr_value = 0;
...
/* @PGE: terminate execution */
  __afl_area_cnt_ptr_value++;
  if (__afl_area_cnt_ptr_value>=__afl_area_cntUp_ptr_value) {
    __afl_area_cnt_ptr[0] = __afl_area_cnt_ptr_value;
#ifndef PGE_PERSISTENT
    _exit(0);
#else
