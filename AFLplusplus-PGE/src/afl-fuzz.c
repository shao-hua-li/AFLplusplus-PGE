...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (
      (opt = getopt(
           argc, argv,
           "+Ab:B:c:CdDe:E:hi:I:f:F:g:G:l:L:m:M:nNOo:p:RQs:S:t:T:UV:WXx:YZr:")) >
      0) {
    ...
    switch (opt) {
      ...
      case 'r': {  /* @PGE: target recall */
        if (!optarg) { FATAL("Wrong usage of -r"); }
        if (sscanf(optarg, "%u", &afl->pge.target_recall) < 1 ||
            optarg[0] == '-') {
  ...
  afl->pge.search_time  = 0;
  afl->pge.search_execs = 0;
  afl->pge.exec_short = 0;
  afl->pge.exec_prefix = 0;
  afl->pge.exec_full = 0;
  afl->pge.hist_prefix = 0;
  afl->pge.hist_num_prefix = 0;
  afl->pge.prefix_len = PGE_MAX;
  afl->fsrv.pge_prefix_len = afl->pge.prefix_len;
  afl->fsrv.pge_cnt_bits =
          pge_shm_init(&afl->pge.shm_cnt, 4, 0);
  afl->fsrv.pge_cntUp_bits =
          pge_shm_init(&afl->pge.shm_cntUp, 4, 1);
  
  afl->pge.curr_trace_bits = ck_alloc(afl->fsrv.map_size * sizeof(u8));
  afl->pge.seen_full = ck_alloc(BUDGET_SIZE_PGE * sizeof(u8));
  afl->pge.seen_prefix = ck_alloc(BUDGET_SIZE_PGE * sizeof(u8));
  afl->pge.seen_prefix_search = ck_alloc(BUDGET_SIZE_PGE * sizeof(u8));
  memset(afl->pge.seen_full, 0, BUDGET_SIZE_PGE);
...
stop_fuzzing:
  ...
  fclose(afl->pge.log_search); // @PGE: cleanup
  fclose(afl->pge.log_exec); // @PGE: cleanup
  afl_shm_deinit(&afl->pge.shm_cnt); // @PGE: cleanup
  afl_shm_deinit(&afl->pge.shm_cntUp); // @PGE: cleanup
  ck_free(afl->pge.curr_trace_bits);
  ck_free(afl->pge.seen_full);
  ck_free(afl->pge.seen_prefix);
  ck_free(afl->pge.seen_prefix_search);
