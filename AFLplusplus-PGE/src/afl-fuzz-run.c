...
common_fuzz_stuff(afl_state_t *afl, u8 *out_buf, u32 len) {
  ...
  if (unlikely(afl->pge.search_phase)) {
    // @PGE: randomly decide if we skip current input.
    if((afl->pge.prefix_len == PGE_MAX) && (rand_below(afl, 100) < SKIP_PROB_PGE)) {
      // afl->stage_cur--; // ignore this input but don't decrease original budget.
      if(afl->stage_cur+1 >= afl->stage_max) {
        if(afl->pge.search_execs >= MIN_EXEC_PGE) {
          return 1;
        } else {
          afl->stage_cur = 0;
          return 0;
    ...
    // @PGE: terminate if our budget is used up.
    if(afl->pge.search_execs >= MAX_EXEC_PGE)
      return 1;
  }

  // save prefix execution in PGE search phase
  if (afl->pge.search_phase && (afl->pge.prefix_len < PGE_MAX) && (afl->pge.prefix_len >= afl->pge.arr_exec_len[afl->pge.search_execs])) {
    // no need run, use cache
    afl->pge.arr_prefix[afl->pge.search_execs] = 1;
    afl->pge.search_execs++;
    return 0;
  }

  afl->fsrv.pge_prefix_len = afl->pge.prefix_len; // setup prefix length
  afl->pge.has_classified = 0;
  ...
  if (unlikely(afl->pge.search_phase)) {
    // @PGE: prefix search phase
    afl->pge.curr_trace_cksum = 0;
    u32 curr_exec_len = *(u32*)afl->fsrv.pge_cnt_bits;
    if (afl->fsrv.pge_prefix_len == PGE_MAX) {
      // @PGE: full execution
      // u8 ifi = if_full_interesting(afl); //defer after save_if_interesting to save some time
      // afl->pge.arr_full[afl->pge.search_execs] = ifi;
      // afl->pge.total_ifi = afl->pge.total_ifi + ifi;
      afl->pge.avg_len += curr_exec_len;
      afl->pge.arr_exec_len[afl->pge.search_execs] = curr_exec_len; 
      afl->pge.arr_out_buf[afl->pge.search_execs] = ck_alloc_nozero(len);
      memcpy(afl->pge.arr_out_buf[afl->pge.search_execs], out_buf, len);
      afl->pge.arr_out_buf_len[afl->pge.search_execs] = len;
      // afl->pge.search_execs++;
      // return 0;
    } else {
      // @PGE: prefix execution
      afl->fsrv.total_execs--; // remove redundant increment
      afl->pge.exec_total--;
      afl->pge.arr_prefix[afl->pge.search_execs] = if_prefix_interesting(afl, afl->pge.seen_prefix_search);;
    }
    if (afl->fsrv.pge_prefix_len != PGE_MAX) {
      afl->pge.search_execs++;
      return 0;
  ...
  if (likely(afl->pge.active)) {
    // @PGE: we're in PGE!
    u8 ipi = if_prefix_interesting(afl, afl->pge.seen_prefix);
    u32 curr_exec_len = *(u32*)afl->fsrv.pge_cnt_bits;
    if (curr_exec_len > (afl->pge.prefix_len+10)) {
      u8 *tmp;
      tmp = alloc_printf("Something maybe went wrong. curr_exec_len: %u, prefix_len: %u\n", curr_exec_len, afl->pge.prefix_len);
      pge_log(afl, PGE_LOG_SEARCH, tmp);
      ck_free(tmp);
    }
    if (curr_exec_len < afl->pge.prefix_len) {
      afl->pge.exec_short++; // target terminated before reaching the target prefix length
    } else if (ipi == 0) {
      afl->pge.exec_prefix++; // prefix not interesting, we saved one full execution!
      goto PGE_PREFIX_END;
    } else {
      afl->pge.exec_full++; // prefix is interesting, run full execution.
      afl->fsrv.pge_prefix_len = PGE_MAX;
      afl->pge.has_classified = 0;
      len = write_to_testcase(afl, (void **)&out_buf, len, 0);
      fault = fuzz_run_target(afl, &afl->fsrv, afl->fsrv.exec_tmout);
      afl->fsrv.total_execs--; // remove redundant increment
  ...
  //turn off PGE
  u8 saved_search_phase = afl->pge.search_phase;
  u8 saved_active = afl->pge.active;
  afl->pge.search_phase = 0;
  afl->pge.active = 0;
  afl->pge.curr_trace_cksum = 0;
  afl->pge.curr_trace_inter = 0;

  afl->queued_discovered += save_if_interesting(afl, out_buf, len, fault);

  //turn on PGE
  afl->pge.search_phase = saved_search_phase;
  afl->pge.active = saved_active;
  
  u8 ifi;
  if (afl->pge.active || afl->pge.search_phase) {
    ifi = if_full_interesting(afl); // mark current full execution
  }
  if (unlikely(afl->pge.search_phase)) {
    // @PGE: prefix search phase
    if (afl->fsrv.pge_prefix_len == PGE_MAX) {
