...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  /* @PGE: setup PGE log files */
  tmp = alloc_printf("%s/pge_search", afl->out_dir);
  int fd = open(tmp, O_WRONLY | O_CREAT | O_EXCL, DEFAULT_PERMISSION);
  if (fd < 0) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);

  afl->pge.log_search = fdopen(fd, "w");
  if (!afl->pge.log_search) { PFATAL("fdopen() failed"); }

  fprintf(
      afl->pge.log_search,
      "# relative_time, search_execs, avg_len, prefix_len, "
      "precision, recall, target_recall, pge_search_time, input_len, input_name\n");
  fflush(afl->pge.log_search);

  tmp = alloc_printf("%s/pge_exec", afl->out_dir);
  fd = open(tmp, O_WRONLY | O_CREAT | O_EXCL, DEFAULT_PERMISSION);
  if (fd < 0) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);

  afl->pge.log_exec = fdopen(fd, "w");
  if (!afl->pge.log_exec) { PFATAL("fdopen() failed"); }

  fprintf(
      afl->pge.log_exec,
      "# relative_time, total_execs_afl, total_execs_pge, full_execs, prefix_execs, short_execs\n");
