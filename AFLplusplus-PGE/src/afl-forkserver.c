...
afl_fsrv_run_target(afl_forkserver_t *fsrv, u32 timeout,
  ...
  if (!fsrv->nyx_mode) {
    ...
    if (fsrv->pge_cnt_bits) { //@PGE: avoid memset in cmplog mode.
      memset(fsrv->pge_cnt_bits, 0, 4); //@PGE: .
      memset(fsrv->pge_cntUp_bits, 0, 4); //@PGE: .
      memcpy(fsrv->pge_cntUp_bits, &fsrv->pge_prefix_len, 4);//@PGE: .
    }
    MEM_BARRIER();
